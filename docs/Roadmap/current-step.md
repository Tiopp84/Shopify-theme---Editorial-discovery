...
| Header scroll compact behavior | `theme/sections/header.liquid`, `theme/assets/header-shell.js` | STATIC PASS — with the existing sticky-header setting enabled, scrolling down past 120 px keeps a centered, short, content-width desktop taskbar at the top: home brand plus primary menu only; search/account/cart are omitted. Its width and height interpolate into the compact state. The small `›` submenu chevron spins on hover/focus and rests down when open; submenu links have hover/focus background, motion and focus-ring feedback. Submenus render only child links explicitly configured in Shopify Navigation; no synthetic parent “View all” link is added. The full header returns only after a deliberate 48 px upward scroll, avoiding state flicker. Existing menu, submenu, search, cart, keyboard and reduced-motion behavior remain in place. Validator and Theme Check pass, 2026-07-28; Shopify preview evidence remains pending. |

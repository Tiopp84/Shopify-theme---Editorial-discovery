...
| Featured edit editorial restyle | `theme/sections/featured-edit.liquid`, `theme/locales/en.default.schema.json` | STATIC PASS — the section is now a full-width editorial edit: eyebrow, campaign heading and merchant-authored description form a left introduction while the optional collection/custom CTA sits to the right with directional-arrow feedback. Cards retain the merchant-selected collection and 3/4-column desktop setting, add a consistent “Look 01 / 04” index, and arrange index, price and product title as a clear compact hierarchy. Each media card receives a restrained framed/dashed treatment; sale status shifts to its upper-right corner. The layout is two columns from 48 rem and uses merchant-selected 3/4 columns from 64 rem; mobile remains two columns with an independent stacked header. Validator and Theme Check pass; Shopify preview/Theme Editor evidence remains pending. |

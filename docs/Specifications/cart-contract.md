...
| Add, change and remove mutation | Standard product/cart forms without JavaScript; Shopify Cart Ajax API only as enhancement | `cart-drawer` custom element |
...
- Quantity, line total and subtotal update from one local drawer state. A customer interaction changes that state immediately. Once the queued Shopify response arrives, the drawer redraws only when Shopify returns a different unit price, cart-level discount or total; this reconciles quantity breaks and automatic discounts without sacrificing immediate feedback.

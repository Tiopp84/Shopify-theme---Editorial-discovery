...
- Theme Blocks are targeted by their parent section allowlist, not merely by being present in `/blocks`. Generic builders must declare a narrow explicit set of generic blocks; they must not become a backdoor for product, footer, navigation, or commerce blocks. Product blocks remain Product-owned, and footer blocks remain Footer-owned. The repository validator enforces placement policy on every Liquid section and rejects generic `@theme` section schemas.
